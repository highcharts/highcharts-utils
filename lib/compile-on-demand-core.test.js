...
describe('createStrictModuleResolutionPlugin', () => {
    test('should create a plugin with correct name', () => {
        const plugin = createStrictModuleResolutionPlugin();
        assert.strictEqual(plugin.name, 'strict-module-resolution');
    });

    test('should have a setup function', () => {
        const plugin = createStrictModuleResolutionPlugin();
        assert.strictEqual(typeof plugin.setup, 'function');
    });

    test('should expose collected errors through the helper', () => {
        const plugin = createStrictModuleResolutionPlugin();
        assert.deepStrictEqual(getStrictModuleResolutionErrors(plugin), []);
    });

    test('should register onResolve hook', () => {
        const plugin = createStrictModuleResolutionPlugin();
        const hooksCalled = [];

        // Create a mock build object to capture hook registrations
        const mockBuild = {
            onResolve: (config, callback) => {
                hooksCalled.push({ config, callback });
            }
        };
        ...
        assert.strictEqual(hooksCalled[0].config.filter.source, '.*');
    });

    test('should collect errors for relative imports without extensions', () => {
        const plugin = createStrictModuleResolutionPlugin();
        let resolveCallback;
        ...
        // Should collect for relative import without extension
        const result0 = resolveCallback({
            path: './utils',
            importer: '/src/index.ts',
            kind: 'import-statement'
        });
        assert.strictEqual(result0, null);
        assert.strictEqual(getStrictModuleResolutionErrors(plugin).length, 1);
        assert.match(
            getStrictModuleResolutionErrors(plugin)[0],
            /Missing file extension in ES module import/
        );

        // Should not throw for relative import with .js extension
        const result1 = resolveCallback({ path: './utils.js', importer: '/src/index.ts', kind: 'import-statement' });
        assert.strictEqual(result1, null);

        // Should not throw for absolute imports
        const result2 = resolveCallback({ path: 'lodash', importer: '/src/index.ts', kind: 'import-statement' });
        assert.strictEqual(result2, null);

        // Should not throw for directory imports
        const result3 = resolveCallback({ path: './utils/', importer: '/src/index.ts', kind: 'import-statement' });
        assert.strictEqual(result3, null);
    });
});

describe('prependModuleResolutionErrors', () => {
    test('should return the original script when there are no errors', () => {
        const js = 'console.log("ok");';
        assert.strictEqual(prependModuleResolutionErrors(js, []), js);
    });

    test('should prepend console.error calls before the compiled script', () => {
        const js = 'console.log("ok");';
        const output = prependModuleResolutionErrors(js, [
            'Missing file extension in ES module import: "./utils"'
        ]);

        assert.match(output, /^console\.error\(/);
        assert.match(output, /Missing file extension in ES module import/);
...
describe('buildEsbuildConfig', () => {
    ...
    test('should include plugins', () => {
        ...
        // Should include strict module resolution plugin + passed plugins
        assert.strictEqual(config.plugins.length, 2);
        assert.strictEqual(config.plugins[0].name, 'strict-module-resolution');
        assert.strictEqual(config.plugins[1].name, 'test-plugin');
    });
