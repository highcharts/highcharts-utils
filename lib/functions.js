...
const getHTML = (req, codePath) => {
	...
	const { rewriteSamplesToES6, theme } = req.session;
...
const getJS = (path, req, codePath, es6Context) => {
	try {
		...
		const { rewriteSamplesToES6 } = req.session;
...
const getCodeFile = async file => {
	...
	if (compileOnDemand) {
		await compile(relativePath);
		console.log('Compiled on demand', relativePath);
	}
