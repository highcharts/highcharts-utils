...
export const replaceURLs = (str, codePath) => {
    ...
    // Controls, loaded from jsdelivr CDN
    ret = ret.replace(
...
export const getHTML = async(req, codePath) => {
    ...
    if (fs.existsSync(configFile) && compileOnDemand) {
        ...
        await saveDemoFile(config).catch(err => {
            if (/tree\.json/.test(err.message)) {
                warning = `Error compiling <code>config.ts</code>.
                    Try running <code>gulp jsdoc</code> in the highcharts repo
                    to generate required <code>tree.json</code> files.`;
            } else {
                warning = `Error compiling config.ts:<br>${err.message}`;
            }
        });
    }
