...
const sampleMainFileCandidates = [
    'config.ts',
    'demo.ts',
    'demo.js',
    'demo.tsx',
...
const reactFallbackImportMap = {
    imports: {
        ...
        'highcharts/': 'https://unpkg.com/highcharts@12.6.0/'
    }
};
...
export const replaceURLs = (str, codePath) => {
    ...
    const { compileOnDemand } = getSettings();
    ...
    // Connectors, external repo
    ret = ret.replace(
        /https:\/\/code\.highcharts\.com\/connectors\/morningstar/g,
    ...
    // Controls, loaded from jsdelivr CDN. If local repository exists, load from
    // there.
    if (
        ret.indexOf(
            'https://cdn.jsdelivr.net/npm/@highcharts/controls'
        ) !== -1 &&
        fs.existsSync(join(highchartsDir, '..', 'highcharts-controls', 'js', 'controls.js'))
    ) {
        ret = ret.replace(
            /https:\/\/cdn\.jsdelivr\.net\/npm\/@highcharts\/controls(@[\d.]+)?/,
            '/highcharts-controls/js/controls.js'
        );
        ret += `<script>console.info(
            '%cLoaded Highcharts Controls from local highcharts-controls/js/controls.js',
            'color:green'
        );</script>`;
    }

    ret = compileOnDemand ?
        // Just fall back to transpiled ES-modules, it's easier
        ret.replace(
            /\/code\/esm\/([a-z\-\/]+)\.js/g,
            '/code/es-modules/masters/$1.src.js'
        ) :
        // ESM files must be built as either .js or .src.js. Only .src.js is
        // built during regular gulp task.
        ret.replace(
            /\/code\/esm\/([a-z\-\/]+)\.js/g,
            '/code/esm/$1.src.js'
        );
...
const handleGeneratorConfig = async(samplePath) => {

    const { compileOnDemand } = getSettings();

    // If a config file exists and we're in compileOnDemand mode, compile first
    const configFile = join(highchartsDir, 'samples', samplePath, 'config.ts');

    let warning;

    if (fs.existsSync(configFile) && compileOnDemand) {
        const generator = await import(
            'file:///' +
            join(highchartsDir, 'tools/sample-generator/index.ts') +
            `?update=${Date.now()}`
        ).catch(err => {
            if (/ERR_UNKNOWN_FILE_EXTENSION/.test(err.stack)) {
                warning = `Cannot load <code>sample-generator/index.ts</code>.
                    TS files require Node.js v24 or higher. Running pre-compiled
                    demo.`;
            }
        });
        ...
        const configModule = await import(
            `file:///${configFile}?update=${Date.now()}`
        ).catch(err => {
            warning = `Error importing <code>config.ts</code>:<br>
                ${err.message}<br>
                <pre>${err.stack.split('\n').slice(0,5).join('<br>')}</pre>`;
        });
        ...
        config.output = samplePath;

        await generator.saveDemoFile(config).catch(err => {
            if (/tree\.json/.test(err.message) || /E2:/.test(err.message)) {
                warning = `Error compiling <code>config.ts</code>.
                    Run <code>gulp scripts && gulp jsdoc-dts</code> in the highcharts repo to
                    generate <code>d.ts</code> files, then restart the
                    utils<br>
                    <small>
                    ${err.message}<br>
                    ${err.stack.split('\n').slice(0,5).join('<br>')}
                ...
                warning = `Error compiling <code>config.ts</code>:<br>
                ${err.message}<br>
                ${err.stack.split('\n').slice(0,5).join('<br>')}`;
            }
        });
...
const getReactFallbackHTML = (mainFile, codePath) => {
    if (!['demo.jsx', 'demo.tsx'].includes(mainFile)) {
        return '';
    }

    const fallbackMap = JSON.stringify(reactFallbackImportMap, null, 4);
    return `
        <script type="importmap">${fallbackMap}</script>
...
export const getSampleEntryInfo = (samplePath) => {
    const sampleRoot = join(samplesDir, samplePath);
    const existingMainFiles = sampleMainFileCandidates.filter(
        fileName => fs.existsSync(join(sampleRoot, fileName))
    );
    const hasDemoHtml = fs.existsSync(join(sampleRoot, 'demo.html'));
    const hasDemoTS = existingMainFiles.includes('demo.ts');

    return {
        sampleRoot,
        hasDemoHtml,
        existingMainFiles,
        mainFile: existingMainFiles[0],
        canView: hasDemoHtml || existingMainFiles.length > 0,
        editableFiles: [
            ...existingMainFiles.filter(fileName => !(fileName === 'demo.js' && hasDemoTS)),
            'demo.html',
            'demo.css',
            'demo.details',
            'readme.md',
...
export const getHTML = async(req, codePath) => {

    const testAutoload = false;

    const { theme, useESModules } = getSettings();

    let samplePath = req.query.path.replace(/^samples\//, '');
    const sampleEntry = getSampleEntryInfo(samplePath);
    let error;
    let file = join(highchartsDir, 'samples', samplePath, 'demo.html');
    let hasForbidden;

    const warning = await handleGeneratorConfig(samplePath);

    let html = getReactFallbackHTML(sampleEntry.mainFile, codePath);
    if (!html) {
        html = `<div class="error-message"><strong>${samplePath}/demo.html</strong>
...
export const getCSS = (req, codePath) => {
    const path = req.query.path.replace(/^samples\//, ''),
...
export const getMainFilename = (req) => {
    const path = req.query.path.replace(/^samples\//, '');
    return getSampleEntryInfo(path).mainFile;
}
...
export const getCodeFile = async (file, req) => {
    ...
    let relativePath = file.split('?')[0].replace('/stock/', '/'),
        path,
        content;
    ...
    if (relativePath.indexOf('/lib/') === 0) {
        ...
        // Load the original CSS files, not the ones that are copied to /code
        if (compileOnDemand && path.endsWith('.css')) {
            path = path
                .replace(
                    '/code/dashboards/css/datagrid.css',
                    '/css/datagrid/datagrid.css'
                )
                .replace('/code/dashboards/css/', '/css/dashboards/')
                .replace('/code/grid/css/', '/css/grid/')

                // Highcharts
                .replace('/code/css/', '/css/');
        }
    }
    ...
    const pathForExist = doCompile ?
        getMasterPath(relativePath) :
        path;

    // /Users/torstein/GitHub/highcharts/ts/masters/vendor/jspdf.ts
    ///Users/torstein/GitHub/highcharts/vendor/jspdf.js
    ...
    if (codeWatch) {
        ...
        if (content && isPrimaryBundle(path)) {
            const codeWatchInclude = await fs.promises.readFile(
                join(dirname(import.meta), 'codewatch.js'),
                'utf8'
            );
            content += `
                ${codeWatchInclude}
                codeWatch(${compileOnDemand});
            `;
        }
    }
