...
export const getJS = (req, codePath, es6Context) => {
    try {
        ...
        const es6FilePath = join(highchartsDir, 'samples', path, 'demo.mjs');

        const esbuildPaths = {
            ts: join(highchartsDir, 'samples', path, 'demo.ts'),
            tsx: join(highchartsDir, 'samples', path, 'demo.tsx'),
            jsx: join(highchartsDir, 'samples', path, 'demo.jsx')
        }
        ...
        if (!fs.existsSync(filePath)) {
            for (const [loader, transpilePath] of Object.entries(esbuildPaths)) {
...
export const getCodeFile = async (file, req) => {
    ...
    const pathForExist = compileOnDemand ?
