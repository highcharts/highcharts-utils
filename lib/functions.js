...
export const getHTML = async(req, codePath) => {
    ...
    const { compileOnDemand, theme, useESModules } = getSettings();
    ...
    if (fs.existsSync(configFile) && compileOnDemand) {
        ...
        if (generator) {
            ...
            const configModule = await import(`${configFile}?update=${Date.now()}`);
            ...
            await generator.saveDemoFile(config).catch(err => {
                if (/tree\.json/.test(err.message) || /E2:/.test(err.message)) {
                    warning = `Error compiling <code>config.ts</code>.
                        ...
                        generate <code>d.ts</code> files.<br>
                        ${err.message}<br>
                        ${err.stack.split('\n').slice(0,5).join('<br>')}`;
                } else {
                    warning = `Error compiling config.ts:<br>
                    ${err.message}<br>
                    ${err.stack.split('\n').slice(0,5).join('<br>')}`;
                }
            });
