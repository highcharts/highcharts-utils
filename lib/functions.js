...
export const getDetails = (path, req) => {
    let details;
    let detailsFile = join(getSamplesDir(), path, 'demo.details');
...
export const getKarmaHTML = () => {
    let files = getLocalJSON(join(getHighchartsDir(), 'test', 'karma-files.json'));
...
export const replaceURLs = (str, codePath) => {
    ...
    if (
        ...
        fs.existsSync(join(getHighchartsDir(), '..', 'highcharts-controls', 'js', 'controls.js'))
    ) {
        ret = ret.replace(
...
const handleGeneratorConfig = async(samplePath) => {
    ...
    const configFile = join(getHighchartsDir(), 'samples', samplePath, 'config.ts');
    ...
    if (fs.existsSync(configFile) && compileOnDemand) {
        const generator = await import(
            'file:///' +
            join(getHighchartsDir(), 'tools/sample-generator/index.ts') +
            `?update=${Date.now()}`
        ).catch(err => {
...
export const getHTML = async(req, codePath) => {
    ...
    let error;
    let file = join(getHighchartsDir(), 'samples', samplePath, 'demo.html');
    let hasForbidden;
...
export const getBranch = () => {
    return branchName({ altPath: getHighchartsDir() });
}

export const getLatestCommit = () => {
    const commit = latestCommit(getHighchartsDir());

    return commit.commit.substr(0, 10);
}

export const getThemes = async () => {
    const files = await fsp.readdir(`${getHighchartsDir()}/ts/Extensions/Themes`),
        { theme } = getSettings();
...
export const getLatestTag = () => latestCommit(getHighchartsDir(), true);

export const getCSS = (req, codePath) => {
    const path = req.query.path.replace(/^samples\//, ''),
        cssFile = join(getHighchartsDir(), 'samples', path, 'demo.css');
    let css = '';
...
export const getMainFilename = (req) => {
    const path = req.query.path.replace(/^samples\//, ''),
        candidates = ['config.ts', 'demo.ts', 'demo.js', 'demo.tsx', 'demo.mjs', 'demo.jsx'];
    for (const fileName of candidates) {
        const filePath = join(getHighchartsDir(), 'samples', path, fileName);
...
export const getJS = (req, codePath, es6Context) => {
    try {
        ...
        const sampleDir = join(getHighchartsDir(), 'samples', path)
...
export const getReadme = (req) => {
    const path = req.query.path.replace(/^samples\//, ''),
        file = join(getHighchartsDir(), 'samples', path, 'readme.md');
...
export const getTestNotes = (req) => {
    ...
    let file = join(getHighchartsDir(), 'samples', path, 'test-notes.md');
    let markdown = '';
    ...
    file = join(getHighchartsDir(), 'samples', path, 'test-notes.html');
...
export const getCodeFile = async (file, req) => {
    ...
    if (relativePath.indexOf('/lib/') === 0) {
        ...
        path = join(getHighchartsDir(), relativePath);
        ...
        path = join(getHighchartsDir(), 'code', relativePath);
