...
export const getJS = (req, codePath, es6Context) => {
    try {
        ...
        const es6FilePath = join(highchartsDir, 'samples', path, 'demo.mjs'),
            tsFilePath = join(highchartsDir, 'samples', path, 'demo.ts');
        ...
        let js;
        if (!fs.existsSync(filePath) && fs.existsSync(tsFilePath)) {
            const ts = fs.readFileSync(tsFilePath, 'utf8');
            js = esbuild.transformSync(ts, {
                loader: 'ts'
            }).code;
        }
