...
const strictModuleResolutionState = new WeakMap();
...
export function getDefaultReplacements(umdConfig) {
    const replacements = {
        ...
        'Data/DataTableCore.ts': undefined,
        ...
        const getOptions = ${umdConfig.name}.getOptions;`,
...
/**
 * Create a strict module resolution plugin that enforces explicit .js extensions
 * in relative imports, similar to webpack's fullySpecified: true.
 * This helps catch issues like the one reported in highcharts/highcharts#24760
 * @returns {Object} esbuild plugin
 */
export function createStrictModuleResolutionPlugin() {
    const missingExtensions = [];

    const plugin = {
        name: 'strict-module-resolution',
        setup(build) {
            build.onResolve({ filter: /.*/ }, (args) => {
                ...
                if (/\.(js|ts|json|css|scss)$/.test(args.path)) {
                    return null;
                }

                // Skip if it's a directory-like import (ends with /)
                if (args.path.endsWith('/')) {
                    return null;
                }

                missingExtensions.push(
                    `Missing file extension in ES module import: "${args.path}"\n` +
                    `  File: ${args.importer}\n` +
                    `  Kind: ${args.kind}\n` +
                    `  Add .js or .ts extension to avoid module resolution issues.`
                );
    ...
    strictModuleResolutionState.set(plugin, missingExtensions);

    return plugin;
}

/**
 * Get collected strict module resolution errors for a plugin instance.
 * @param {Object} plugin - Strict module resolution plugin instance
 * @returns {Array<string>} Collected missing extension errors
 */
export function getStrictModuleResolutionErrors(plugin) {
    return strictModuleResolutionState.get(plugin) || [];
}

/**
 * Prepend strict module resolution errors as runtime console.error calls.
 * @param {string} js - The compiled JavaScript output
 * @param {Array<string>} missingExtensions - Collected module resolution errors
 * @returns {string} JavaScript prefixed with console.error calls when needed
 */
export function prependModuleResolutionErrors(js, missingExtensions) {
    ...
    return missingExtensions
        .map(message => `
            window.hcUtilsWarnings ||= {};
            if (!window.hcUtilsWarnings['${JSON.stringify(message)}']) {
                console.error(${JSON.stringify(message)});
            }
            window.hcUtilsWarnings['${JSON.stringify(message)}'] = true;
        `)
        .join('\n') + '\n' + js;
}
...
export function buildEsbuildConfig(masterPath, umdConfig, isPrimary, plugins) {
    ...
    // Add strict module resolution plugin to catch missing .js extensions
    const strictResolutionPlugin = createStrictModuleResolutionPlugin();
    const allPlugins = [strictResolutionPlugin, ...plugins];
...
export async function compile(filename, options) {
    ...
    const strictResolutionPlugin = esbuildConfig.plugins.find(
        plugin => plugin.name === 'strict-module-resolution'
    );
    ...
    if (result) {
        js = prependModuleResolutionErrors(
            result.outputFiles[0].text,
            getStrictModuleResolutionErrors(strictResolutionPlugin)
        );
        ...
        if (isPrimary) {
            ...
            (() => {
                if (console.hasBeenMonkeyPatchedByHighchartsUtils) {
                    return;
                }
                ...
                console.hasBeenMonkeyPatchedByHighchartsUtils = true;
            })();
            `;
        }
...
export default {
    ...
    generateUMDFooter,
    getLegacyPlugins,
    createStrictModuleResolutionPlugin,
    getStrictModuleResolutionErrors,
    prependModuleResolutionErrors,
    buildEsbuildConfig,
    compile
};
