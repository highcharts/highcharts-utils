 ...
 */
const getPlugins = async (resolvedHighchartsDir) => {
    const pckg = getLocalJSON(join(resolvedHighchartsDir, 'package.json'));
...
export const getMasterPath = (filename) => {
    return getMasterPathCore(filename, getHighchartsDir());
};
...
export const compile = async (filename) => {
    const resolvedHighchartsDir = getHighchartsDir();
    const result = await compileCore(filename, {
        highchartsDir: resolvedHighchartsDir,
        branchName: getBranch(),
        getPlugins: () => getPlugins(resolvedHighchartsDir)
    });

    // Add batched console logging for better UX when multiple files are compiled
    const shortFilename = filename.replace('.src.js', '.js');
    const batchedLogging = `
        window.compiledOnDemand ||= [];
        window.compiledOnDemand.push('${shortFilename} (${result.duration} ms)');
        clearTimeout(window.compiledOnDemandTimeout);
        window.compiledOnDemandTimeout = setTimeout(() => {
            console.info(
                '%cCompiled on demand: ' + window.compiledOnDemand.join(', '),
                'color:green'
            );
            delete window.compiledOnDemand;
        }, 150);
    `;

    // Replace the simple console.info from core with batched logging
    const code = result.code.replace(
        /\s*console\.info\(\s*'%cCompiled on demand:[^']+',\s*'color:green'\s*\);/,
        batchedLogging
    );
