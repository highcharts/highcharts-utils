...
const replace = js => js
    ...
    // Remove the Globals file to avoid overwriting of objects
    .replace(
        /(\/\/ \.\.\/highcharts\/ts\/Core\/Globals\.ts)[\s\S]+?(\/\/ \.\.\/highcharts\/ts\/)/,
        `$1
    ...
    // Remove the SVGRenderer file to avoid overwriting of objects
    .replace(
        /(\/\/ \.\.\/highcharts\/ts\/Core\/Renderer\/SVG\/SVGRenderer\.ts)[\s\S]+?(\/\/ \.\.\/highcharts\/ts\/)/,
        `$1
        var SVGRenderer = Highcharts.SVGRenderer,
            SVGRenderer_default = SVGRenderer;
...
const compile = async (path) => {
    ...
    const result = await esbuild.build({
        ...
        bundle: true,
        write: false,
        globalName: isMainFile ? 'Highcharts' : 'HighchartsModule',
        banner: {
            js: isMainFile ?
